...
const Switch = ({ isOn, handleToggle, className, label }: SwitchProps) => {
  const classNames = ['switch', isOn ? 'switch_is-on' : 'switch_is-off'].join(' ');
  return (
    <SwitchWrapper className={`mvm-switch ${className || ''}`}>
      <div className={classNames} onClick={handleToggle} onKeyPress={handleToggle} tabIndex={0} role="button">
